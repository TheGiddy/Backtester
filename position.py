...
class Position(object):
    def __init__(self, action, ticker, init_quantity, init_price, init_commission, bid, ask):
        '''
        Set up the initial "account" of the Position to be zero for most items, with the exception of the initial
        purchase/sale. Then calculate the initial values and finally update the market value of the transaction.
    ...
    def _calculate_initial_value(self):
        '''
        Depending on whether the action was a buy or sell ('BOT' or 'SLD') calculate the average bought cost, the total
        bought cost, the average price and the cost basis.
    ...
    def update_market_value(self, bid, ask):
        '''
        The market value is tricky to calculate as we only have access to the top of the order book through Questrade,
        which means that the true redemption price is unknown until executed. However, it can be estimated via the
        mid-price of the bid-ask spread. Once the market value is calculated it allows calculation of the unrealised
        and realised profit and loss of any transactions.
    ...
    def transact_shares(self, action, quantity, price, commission):
        '''
        Calculates the adjustments to the Position that occur once new shares are bought and sold. Takes care to update
        the average bought/sold, total bought/sold, the cost basis and PnL calculations, as carried out through
        Questrade.
        ...
        # Action == 'SLD'
            ...
            if self.action != 'BOT':  # Increasing short position
                self.avg_price = (self.avg_price * self.sells + price * quantity) // (self.sells + quantity)
        ...
        # Adjust net values, including commission
        self.net = self.buys - self.sells
